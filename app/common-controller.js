...
angular.module('email').factory('utils', ['$filter', 'alertify', 'variables', function ($filter, alertify, variables) {
 ...
 /**
     ...
     * @example: <input type="text" md-input />
     */
    angular.module('email').directive('emailBuilderView', ['utils', 'storage', 'dragulaService', '$interpolate', '$translate', '$templateCache', 'variables', '$location', '$routeParams',
    function (utils, storage, dragulaService, $interpolate, $translate, $templateCache, variables, $location, $routeParams) {
        return {
            restrict: 'E',
            templateUrl: `../views/email-builder.html`,
            scope: true,
            controller: ["$scope", function ($scope) {
                ...
                $scope.saveEmailTemplate = function () {
                    console.log("$scope save", $scope.Email);
                    utils.createEmail($scope.Email).then(function (res) {
                        if (res.errors.length) {
                            ...
                            console.log("$scope save after", $scope.Email);
                            if($scope.editId){

                                $scope.emailTemplates = JSON.parse(localStorage.getItem('Email'));
                                angular.forEach($scope.emailTemplates, function(index, item){

                                    if($scope.editId == item.id){
                                        $scope.emailTemplates[index] = $scope.Email;
                                    }

                                    console.log(" $scope.EmailTemplatessssss", $scope.EmailTemplates);

                                });
                                $scope.Email.dateModified = new Date();

                            }else{

                            $scope.Email.dateCreated = new Date();
                            var timeStamp = new Date(new Date()).getTime();
                            $scope.Email.id = timeStamp;

                            $scope.EmailTemplates.push($scope.Email);

                            }
                            console.log(" $scope.Email", $scope.Email);
                            storage.put($scope.EmailTemplates).then(function () {
